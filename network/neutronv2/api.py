...
class API(base.Base):
    ...
    def update_instance_vif(self, context, instance, vif, index):
        self._refresh_neutron_extensions_cache(context)
        if 'VNIC Index' in self.extensions:
            neutron = neutronv2.get_client(context)
            port_req_body = {'port': {'vnic_index': index}}
            try:
                neutronv2.get_client(context).update_port(vif['id'],
                                                          port_req_body)
            except Exception as e:
                LOG.error(_('Unable to update instance for port %(id)s. '
                            'Exception: %(e)s'),
                          {'id': vif['id'], 'e': e},
                          instance=instance)
