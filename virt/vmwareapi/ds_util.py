...
def get_datastore_by_ref(session, ds_ref):
    ...
    return ds_obj.Datastore(ds_ref, query["summary.name"],
                            ...
                            type=query["summary.type"])


def get_datastore(session,
                  cluster=None,
                  host=None,
    ...
    best_match = None
    while data_stores:
        best_match = _select_datastore(session, data_stores, best_match,
                                       datastore_regex,
                                       allowed_ds_types,
    ...
    if storage_policy is not None:
        raise exception.DatastoreNotFound(
            ...
            % storage_policy)
    elif datastore_regex is not None:
        raise exception.DatastoreNotFound(
...
def _get_allowed_datastores(data_stores, datastore_regex, allowed_types):
    ...
    for obj_content in data_stores.objects:
        ...
        if (propdict['summary.accessible'] and ds_type in allowed_types):
            if datastore_regex is None or datastore_regex.match(ds_name):
                allowed.append(ds_obj.Datastore(obj_content.obj, ds_name))
...
def get_datastore_by_name(session, ds_refs, ds_name):
    """Returns the datastore ref for a requested datstore name."""
    for ds_ref in ds_refs:
        datastore = get_datastore_by_ref(session, ds_ref)
...
def disk_copy(session, dc_ref, src_file, dst_file):
    ...
    LOG.info(_("Copied virtual disk from %(src)s to %(dst)s."),
             {'src': src_file, 'dst': dst_file})
...
def disk_delete(session, dc_ref, file_path):
    ...
    LOG.info(_("Deleted virtual disk %s."), file_path)
...
def _filter_datastores_matching_storage_policy(session, data_stores,
    ...
    if profile_id:
        ...
        if matching_hubs:
            ...
            return data_stores
    LOG.error(_("Unable to retrieve storage policy with name %s"),
              storage_policy)
