...
class VMwareVMOps(object):
    ...
    def attach_block_devices(self, vm_ref, data_store_ref,
                             block_device_mapping, instance):
    ...
    def _configure_config_drive(self, instance, vm_ref, dc_info, datastore,
        ...
        dc_path = oslo_vim_util.get_inventory_path(session_vim, dc_info.ref)
    ...
    def _create_folders(self, parent_folder, folders):
        path_list = []
    ...
    def _get_folder_name(self, name, id):
        # Maximum folder length must be less than 80 characters. 'id' length
        # is 36 maximum prefix for name is 40
        if name:
            return '%s (%s)' % (name[:40], id[:36])
        else:
            return id[:36]

    def _get_instance_metadata(self, context, instance, flavor):
        return ('name:%s\n'
                'userid:%s\n'
                'username:%s\n'
                'projectid:%s\n'
    ...
    def build_virtual_machine(self, context, instance, image_info, dc_info,
        ...
        storage_policy = extra_specs.storage_policy
        if storage_policy:
        ...
        # This is due to the fact that cinder does not have the project name
        folder_name = self._get_folder_name('Project',
                                            context.project_id)
        folder = self._create_folders(dc_info.vmFolder,
                                      ['OpenStack', folder_name, 'Instances'])
        # Create the VM
        vm_ref = vm_util.create_vm(self._session, instance, folder,
                                   config_spec, self._root_resource_pool)
        return vm_ref

    def _get_extra_specs(self, instance_type_id, flavor=None):
        # Read flavors for allocations if they do not exist
        if not flavor:
        ...
        extra_config = flavor['extra_specs'].get('vmware:extra_config')
        if extra_config:
            try:
                extra_specs.extra_config = jsonutils.loads(extra_config)
            except ValueError:
                reason = _('extra_config has invalid format')
    ...
    def _fetch_vsphere_image(self, context, vi, image_ds_loc):
        ...
        datacenter_moref = self._session._call_method(
            self._session._get_vim(),
            "FindByInventoryPath",
            service_content.searchIndex,
            inventoryPath=datacenter_path)

        copy_task = self._session._call_method(
            self._session._get_vim(),
            ...
            destinationDatacenter=vi.dc_info.ref)
        self._session._wait_for_task(copy_task)
    ...
    def _fetch_image_as_file(self, context, vi, image_ds_loc):
        ...
        LOG.debug("Downloading image file data %(image_id)s to "
                  ...
                  {'image_id': vi.ii.image_id,
                   ...
                   'datastore_name': vi.datastore.name},
                  instance=vi.instance)
        dc_path = oslo_vim_util.get_inventory_path(session_vim, vi.dc_info.ref)
    ...
    def _fetch_image_as_vapp(self, context, vi, image_ds_loc):
        ...
        image_size = vmware_images.fetch_image_stream_optimized(
            context,
            vi.instance,
            self._session,
            vm_name,
    ...
    def _fetch_image_as_ova(self, context, vi, image_ds_loc):
        """Download OVA image to host as a vApp."""

        # The directory of the imported disk is the unique name
        # of the VM use to import it with.
        vm_name = image_ds_loc.parent.basename

        LOG.debug("Downloading OVA image %(image_id)s to "
                  "%(file_path)s on the data store "
                  "%(datastore_name)s as vApp",
                  {'image_id': vi.ii.image_id,
                   'file_path': image_ds_loc,
        ...
        image_size = vmware_images.fetch_ova_image(
            context,
            vi.instance,
            self._session,
            vm_name,
    ...
    def _cache_stream_optimized_image(self, vi, tmp_image_ds_loc):
        ...
        try:
            ...
            LOG.debug("File %s already exists", dst_path)
    ...
    def _get_vm_config_info(self, instance, image_info, storage_policy=None):
        ...
        if (instance.root_gb != 0 and
            ...
            raise exception.InstanceUnacceptable(instance_id=instance.uuid,
                                                 reason=reason)
        allowed_ds_types = VMwareVMOps.get_allowed_datastore_types(image_info)
        datastore = ds_util.get_datastore(
                self._session, self._cluster, None, self._datastore_regex,
                allowed_ds_types, storage_policy=storage_policy)
    ...
    def _get_image_callbacks(self, vi):
        ...
        container_format = vi.ii.container_format

        if container_format == constants.CONTAINER_FORMAT_OVA:
        ...
        elif disk_type in constants.DISK_TYPE_STREAM_OPTIMIZED:
    ...
    def _create_ephemeral(self, bdi, instance, vm_ref, dc_info,
        ...
        if not ephemerals and instance.ephemeral_gb:
            ...
            path = str(ds_obj.DatastorePath(datastore.name, folder,
                                            filename))
            self._create_and_attach_ephemeral_disk(instance, vm_ref, dc_info,
                                                   size, adapter_type,
                                                   path)
    ...
    def spawn(self, context, instance, image_meta, injected_files,
        ...
        client_factory = self._session._get_vim().client.factory
        image_info = vmware_images.VMwareImage.from_image(context,
                                                          instance.image_ref,
                                                          image_meta)
        ...
        extra_specs = self._get_extra_specs(instance['instance_type_id'],
                                            flavor=flavor)

        vi = self._get_vm_config_info(
            instance, image_info, storage_policy=extra_specs.storage_policy)
        ...
        vm_ref = self.build_virtual_machine(context,
                                            instance,
                                            image_info,
                                            vi.dc_info,
                                            vi.datastore,
        ...
        # Cache the vm_ref. This saves a remote call to the VC.
        ...
        if len(block_device_mapping) > 0:
            ...
            for disk in block_device_mapping:
                ...
                if disk.get('boot_index') == 0:
                    self._volumeops.attach_root_volume(
                        connection_info, instance, vi.datastore.ref,
                        adapter_type)
                else:
        ...
        self._create_ephemeral(block_device_info, instance, vm_ref, vi.dc_info,
        ...
        # that all of the files for the instance ar under the directory
        # 'uuid' of the instance
        vm_util.rename_vm(self._session, vm_ref, instance)

        vm_util.power_on_instance(self._session, instance, vm_ref=vm_ref)

        if (self._is_neutron and network_info and
                hasattr(self._virtapi, '_compute') and
                hasattr(self._virtapi._compute, 'network_api')):
            for index, vif in enumerate(network_info):
                self._virtapi._compute.network_api.update_instance_vif(
    ...
    @staticmethod
    def get_allowed_datastore_types(image_info):
        if (image_info.disk_type == constants.DISK_TYPE_STREAM_OPTIMIZED or
                image_info.is_iso):
            return ds_util.ALL_SUPPORTED_DS_TYPES
        return ds_util.NON_STREAMABLE_DS_TYPES
    ...
    def _create_linked_clone_from_snapshot(self, instance,
        ...
        vm_name = "%s_%s" % (constants.SNAPSHOT_VM_PREFIX, salt)
    ...
    def snapshot(self, context, instance, image_id, update_task_state):
        ...
        def _get_vm_and_vmdk_attribs():
            lst_properties = ["summary.config.guestId",
                              "datastore"]
            ...
            return (vmdk.path, vmdk.adapter_type, vmdk.disk_type,
                    datastores, os_type, vmdk.capacity_in_bytes)
        ...
        try:
            ...
            vmware_images.upload_image_stream_optimized(
                context, image_id, instance, self._session, vm=snapshot_vm_ref,
                os_type=os_type, adapter_type=adapter_type,
                vmdk_size=capacity_bytes, data_center_name=dc_info.name,
    ...
    def _destroy_instance(self, instance, network_info, destroy_disks=True):
        ...
        try:
            ...
            if vm_config_pathname:
                vm_ds_path = ds_obj.DatastorePath.parse(vm_config_pathname)
            ...
            # Get the datastore for the instance
            ds_ref_ret = query['datastore']
            ds_ref = None
            if vm_ds_path:
                # There may be more than one datastore that is configured on
                # the VM, for example if there is a cinder volume attached. In
                # this case we return the specific requested datastore.
                ds_ref = ds_util.get_datastore_by_name(
                    self._session, ds_ref_ret.ManagedObjectReference,
                    vm_ds_path.datastore)

            vm_util.destroy_vm(self._session, instance, vm_ref)

            if destroy_disks and vm_ds_path and ds_ref:
                try:
                    ...
                    # Ensure that exceptions do not appear in VC
                    if ds_util.path_exists(self._session, ds_browser,
    ...
    def pause(self, instance):
        # VC does not support pause. A suspend will be done here instead.
        LOG.warning(_("Pause is not supported. Suspending instead."),
                    instance=instance)
        self.suspend(instance)

    def unpause(self, instance):
        # VC does not support unpause. A resume will be done here instead.
        LOG.warning(_("Unpause is not supported. Resuming instead."),
                    instance=instance)
    ...
    def rescue(self, context, instance, network_info, image_meta):
        """Rescue the specified instance. Attach the image that the instance
        was created from and boot from it.
        """
        vm_ref = vm_util.get_vm_ref(self._session, instance)
        vmdk = vm_util.get_vmdk_info(self._session, vm_ref,
                                     uuid=instance['uuid'])
        ...
        hardware_devices = self._session._call_method(vim_util,
                        "get_dynamic_property", vm_ref,
                        "VirtualMachine", "config.hardware.device")
        rescue_device = vm_util.find_rescue_device(hardware_devices)
        factory = self._session._get_vim().client.factory
    ...
    def _resize_vm(self, context, vm_ref, instance, flavor):
    ...
    def _resize_disk(self, instance, vm_ref, vmdk, flavor):
        if (flavor['root_gb'] > instance['root_gb'] and
            ...
            ds_util.disk_copy(self._session, dc_info.ref, vmdk.path,
                              str(resized_disk))
            self._extend_virtual_disk(instance, root_disk_in_kb,
                                      resized_disk, dc_info.ref)
    ...
    def migrate_disk_and_power_off(self, context, instance, dest,
        ...
        self._resize_vm(context, vm_ref, instance, flavor)
        self._update_instance_progress(context, instance,
                                       step=2,
                                       total_steps=RESIZE_TOTAL_STEPS)
    ...
    def _resize_original_disk_exists(self, ds_ref, original_disk):
        ds_browser = self._get_ds_browser(ds_ref)
        return ds_util.file_exists(self._session, ds_browser,
    ...
    def confirm_migration(self, migration, instance, network_info):
        ...
        original_disk = ds_obj.DatastorePath(datastore, folder,
                                              'original.vmdk')
        if self._resize_original_disk_exists(ds_ref, original_disk):
    ...
    def finish_revert_migration(self, context, instance, network_info,
        ...
        if self._resize_original_disk_exists(ds_ref, original_disk):
            ...
            ds_util.disk_move(self._session, dc_info.ref,
                              vmdk.path, resized_disk)
            ds_util.disk_move(self._session, dc_info.ref,
                              str(original_disk), vmdk.path)
            self._volumeops.attach_disk_to_vm(vm_ref, instance,
                                              vmdk.adapter_type,
                                              vmdk.disk_type, vmdk.path)
            ds_util.disk_delete(self._session, dc_info.ref, resized_disk)
    ...
    def finish_migration(self, context, migration, instance, disk_info,
        ...
        # Update ephemerals if necessary
        self._resize_create_ephemerals(vm_ref, instance, block_device_info)
    ...
    def _get_vnc_console_connection(self, instance):
        ...
        props = self._session._call_method(vim_util,
                               ...
                               [vm_util.VNC_CFG_PORT_KEY,
                                vm_util.VNC_CFG_PASSWORD_KEY])
        opt_port = props.get(vm_util.VNC_CFG_PORT_KEY)
        opt_pass = props.get(vm_util.VNC_CFG_PASSWORD_KEY)

        if opt_port:
            port = int(opt_port.value)
        else:
            raise exception.ConsoleTypeUnavailable(console_type='vnc')

        vnc_password = opt_pass.value if opt_pass else None
    ...
    def _check_if_folder_file_exists(self, ds_browser, ds_ref, ds_name,
        ...
        folder_ds_path = ds_obj.DatastorePath(ds_name, folder_name)
    ...
    def _get_shadow_vm_name(self, vi):
        image_size = '-%sG' % vi.root_gb
        max_name_len = 41 - len(image_size)
        image_name = vi.ii.name[:max_name_len] if vi.ii.name else 'image'
        return '%s%s (%s)' % (image_name, image_size, vi.ii.image_id)

    def _create_shadow_vm(self, vi, disk_path):
        LOG.debug("Creating shadow VM for image: %s", str(disk_path))
        name = self._get_shadow_vm_name(vi)
        if vm_util.shadow_vm_exists(self._session, vi.ii.image_id,
                                    disk_path):
            LOG.debug("Shadow VM already exist for image: %s", str(disk_path))
            return

        client_factory = self._session._get_vim().client.factory
        config_spec = vm_util.get_shadow_vm_create_spec(client_factory,
                                                        name,
                                                        vi.ii,
                                                        disk_path)
        ds_folder_name = vm_util.get_datastore_shadow_vm_folder(vi.datastore)
        image_name = vi.ii.name if vi.ii.name else 'image'
        folder_name = self._get_folder_name(image_name, vi.ii.image_id)
        folder = self._create_folders(vi.dc_info.vmFolder,
                                      folders=['OpenStack',
                                               'Images',
                                               ds_folder_name,
                                               folder_name])
        try:
            # create the shadow VM in the corresponding image folder
            vm_util.create_vm(self._session, None, folder, config_spec,
                              self._root_resource_pool)
    ...
    def _use_disk_image_as_linked_clone(self, vm_ref, vi):
        ...
        with lockutils.lock(str(sized_disk_ds_loc),
            ...
            self._create_shadow_vm(vi, sized_disk_ds_loc)
    ...
    def get_host_uptime(self):
        return vm_util.get_host_uptime_from_cluster(self._session,
                                                    self._cluster)
