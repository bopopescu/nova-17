...
class ImageCacheManager(imagecache.ImageCacheManager):
    ...
    def _destroy_shadow_vms(self, image, datastore):
        # get the folder which contains the shadow VMs for this image
        # TODO(rgerganov) add cache for image -> folder_ref
        folder_ref = vm_util.get_shadow_folder_ref(self._session, image,
                                                   datastore)
        if folder_ref is None:
            # only full clones are created from this image
            LOG.debug("There is no vCenter folder for image '%s'", image)
            return
        # Remove the folder_ref from the cache.
        vm_util.folder_ref_cache_delete(folder_ref)
        # destroying the folder will destroy all shadow VMs
        destroy_task = self._session._call_method(self._session._get_vim(),
                                                  "Destroy_Task",
                                                  folder_ref)
                ...
                if not ts:
                    ...
                    if timeutils.is_older_than(dt, age_seconds):
                        LOG.info(_("Image %s is no longer used. "
                                   "Deleting!"), path)
                        # Image has aged - destroy the shadow VMs, this will
                        # also delete the image itself
                        self._destroy_shadow_vms(image, datastore)
                        # delete the datastore folder (should be empty)
