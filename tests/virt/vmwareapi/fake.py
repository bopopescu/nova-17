...
class VirtualMachine(ManagedObject):
    ...
    def __init__(self, **kwargs):
        ...
        managedBy = kwargs.get("managedBy")
        if managedBy:
            self.set("managedBy.extensionKey", managedBy.get("extensionKey"))
            self.set("managedBy.type", managedBy.get("type"))
    ...
    def reconfig(self, factory, val):
        ...
        try:
            ...
            ds = self.get("datastore").ManagedObjectReference[0]
            disk_backing = VirtualDiskFlatVer2BackingInfo(datastore=ds)
...
class Datacenter(ManagedObject):
    ...
    def __init__(self, name="ha-datacenter", ds_ref=None):
        ...
        if _db_content.get("Folder", None) is None:
            create_folder()
        folder_ref = _db_content["Folder"][_db_content["Folder"].keys()[0]].obj
...
def fake_fetch_image(
        context, session, instance, host, port, dc_name, datastore,
        file_path, cookies=None):
    """Fakes the fetch of an image."""
    ds_file_path = "[" + datastore.name + "] " + file_path
    _add_file(ds_file_path)
...
class FakeVim(object):
    ...
    def _add_new_vm(config_spec):
        ...
        vm_path = ds_obj.DatastorePath.parse(config_spec.files.vmPathName)
        ...
        managedBy = {'extensionKey': getattr(config_spec.managedBy,
                                             'extensionKey', None),
                     'type': getattr(config_spec.managedBy,
                                     'type', None)}
        ...
        vm_dict = {"name": config_spec.name,
                  ...
                  "extra_config": getattr(config_spec, 'extraConfig', None),
    ...
    def _create_vm(self, method, *args, **kwargs):
        ...
        vm_obj = FakeVim._add_new_vm(config_spec)
        res_pool = _get_object(pool)
        res_pool.vm.ManagedObjectReference.append(vm_obj)
        task_mdo = create_task(method, "success", result=vm_obj)
        return task_mdo.obj
    ...
    def _clone_vm(self, method, *args, **kwargs):
        ...
        if clone_spec.config is not None:
            ...
            managedBy = {'extensionKey': getattr(clone_spec.config.managedBy,
                                                 'extensionKey', None),
                         'type': getattr(clone_spec.config.managedBy,
                                         'type', None)}

            vm_dict['managedBy'] = managedBy
        virtual_machine = VirtualMachine(**vm_dict)
        _create_object("VirtualMachine", virtual_machine)
        task_mdo = create_task(method, "success")
        return task_mdo.obj
