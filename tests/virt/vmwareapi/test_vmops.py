...
class VMwareVMOpsTestCase(test.NoDBTestCase):
    def setUp(self):
        ...
        fake_ds_ref = vmwareapi_fake.create_datastore('fake_ds', 1024, 512)
        vmwareapi_fake.create_datacenter('fake-dc', fake_ds_ref)
        ...
        with mock.patch.object(utils, 'is_neutron', return_value=False):
        ...
        self._metadata = ("name:fake_display_name\n"
                          "userid:fake_user\n"
                          "username:None\n"
    ...
    def test_is_neutron_nova(self):
        ...
        ops = vmops.VMwareVMOps(mock.Mock(), None, None)
    ...
    @mock.patch('nova.virt.vmwareapi.vm_util.rename_vm')
    ...
    def test_spawn_non_root_block_device(self, from_image,
        ...
        get_extra_specs.return_value = extra_specs
        ...
        get_vm_config_info.return_value = vi
    ...
    def test_spawn_with_no_image_and_block_devices(self, from_image,
        ...
        _metadata = ("name:fake_display_name\n"
                     "userid:fake_user\n"
                     "username:None\n"
                     "projectid:fake_project\n"
                     "projectname:None\n"
        ...
        with mock.patch.object(self._vmops, '_volumeops') as volumeops:
            ...
            from_image.assert_called_once_with(self._context,
                                               instance.image_ref, {})
            get_vm_config_info.assert_called_once_with(instance,
                                                       image_info,
                                                       storage_policy=None)
            build_virtual_machine.assert_called_once_with(
    ...
    def _test_get_datacenter_ref_and_name(self, ds_ref_exists=False):
        ...
        _vmops = vmops.VMwareVMOps(self._session, None, None)
        if ds_ref_exists:
            ds_ref = mock.Mock()
            ds_ref.value = "ds-1"
        else:
        ...
        with mock.patch.object(self._session, '_call_method',
            ...
            dc_info = _vmops.get_datacenter_ref_and_name(instance_ds_ref)

            if ds_ref:
                self.assertEqual(1, len(_vmops._datastore_dc_mapping))
    ...
    def test_rescue(self, mock_get_ds_by_ref, mock_power_on, mock_reconfigure,
                    mock_get_boot_spec, mock_find_rescue, mock_glance,
        ...
        with contextlib.nested(
            ...
            mock.patch.object(self._vmops, 'power_off'),
            mock.patch.object(vm_util, 'get_vmdk_info',
                              return_value=vmdk)
        ) as (_get_dc_ref_and_name, _power_off, fake_vmdk_info):
            _get_dc_ref_and_name.return_value = dc_info
            self._vmops.rescue(self._context, self._instance, None, None)

            _power_off.assert_called_once_with(self._instance)
    ...
    def _test_unrescue(self, power_on):
        ...
        with contextlib.nested(
            ...
            _get_vm_ref.assert_called_once_with(self._session,
                                                self._instance)
            _power_off.assert_called_once_with(vm_ref)
    ...
    def _test_finish_migration(self, power_on=True, resize_instance=False):
        with contextlib.nested(
            ...
            fake_update_instance_progress.called_once_with(
                self._context, self._instance, 5, vmops.RESIZE_TOTAL_STEPS)
    ...
    @mock.patch.object(vmops.VMwareVMOps, '_resize_original_disk_exists',
                       return_value=True)
    @mock.patch.object(vm_util, 'reconfigure_vm')
    ...
    def _test_finish_revert_migration(self, fake_power_on,
                                      ...
                                      fake_disk_delete, fake_get_extra_specs,
        ...
        with contextlib.nested(
            ...
            fake_get_vm_ref.assert_called_once_with(self._session,
                                                    self._instance)
            fake_power_off.assert_called_once_with(self._instance)
            ...
            fake_original_exists.assert_called_once_with(
                'fake-ref', ds_obj.DatastorePath(datastore.name, 'uuid',
                                                 'original.vmdk'))
            ...
            fake_disk_delete.assert_called_once_with(
                self._session, dc_info.ref, '[fake] uuid/resized.vmdk')
    ...
    def test_resize_vm(self, fake_resize_spec, fake_reconfigure,
        ...
        extra_specs = vm_util.ExtraSpecs()
        fake_get_extra_specs.return_value = extra_specs
        flavor = {'vcpus': 2, 'memory_mb': 4096, 'id': 2, 'name': 'm1.micro'}
        self._vmops._resize_vm(self._context, 'vm-ref', self._instance, flavor)
        fake_resize_spec.assert_called_once_with(
            self._session.vim.client.factory, 2, 4096, extra_specs,
            metadata=self._metadata)
    ...
    @mock.patch.object(vmops.VMwareVMOps, '_resize_original_disk_exists',
                       return_value=True)
    @mock.patch.object(vm_util, 'get_vm_ref', return_value='fake-ref')
    def test_confirm_migration(self, fake_get_vm_ref, fake_original_exists,
                               fake_disk_delete):
    ...
    def test_migrate_disk_and_power_off(self, fake_get_vm_ref, fake_progress,
        ...
        fake_power_off.assert_called_once_with(self._instance)
        fake_resize_vm.assert_called_once_with(self._context, 'fake-ref',
                                               self._instance, flavor)
        fake_resize_disk.assert_called_once_with(self._instance, 'fake-ref',
                                                 vmdk, flavor)
        fake_remove_ephemerals.assert_called_once_with('fake-ref')
        calls = [
                mock.call(self._context, self._instance, step=0,
                          total_steps=5),
                mock.call(self._context, self._instance, step=1,
                          total_steps=5),
                mock.call(self._context, self._instance, step=2,
                          total_steps=5),
                mock.call(self._context, self._instance, step=3,
                          total_steps=5),
                mock.call(self._context, self._instance, step=4,
                          total_steps=5)]
        self.assertEqual(calls,
                         fake_progress.call_args_list)

    @mock.patch(
        'nova.virt.vmwareapi.vmops.VMwareVMOps._create_folders',
        return_value='fake_vm_folder')
    @mock.patch.object(vmops.LOG, 'debug')
    @mock.patch.object(vmops.VMwareVMOps, '_create_folder_if_missing')
    @mock.patch.object(vmops.VMwareVMOps, '_get_vm_config_info')
    @mock.patch.object(vmops.VMwareVMOps, 'build_virtual_machine')
    @mock.patch.object(vmware_images.VMwareImage, 'from_image')
    ...
    @mock.patch('nova.virt.vmwareapi.vmops.VMwareVMOps._create_shadow_vm')
    @mock.patch.object(
    ...
    def _test_use_disk_image_as_linked_clone(self,
        ...
        mock_create_shadow_vm.assert_called_once_with(
                    vi, sized_cached_image_ds_loc)
    ...
    def _verify_spawn_method_calls(self, mock_call_method, extras=None):
        ...
        expected_methods.append('ReconfigVM_Task')
    ...
    @mock.patch(
        'nova.virt.vmwareapi.imagecache.ImageCacheManager.enlist_image')
    @mock.patch(
        'nova.virt.vmwareapi.vmops.VMwareVMOps._get_and_set_vnc_config')
    @mock.patch(
        'nova.virt.vmwareapi.vmops.VMwareVMOps._create_folders',
        return_value='fake_vm_folder')
    @mock.patch('nova.virt.vmwareapi.vmops.VMwareVMOps._create_shadow_vm')
    ...
    def _test_spawn(self,
                   ...
                   mock_create_shadow_vm,
                   mock_prepare_instance_folders,
                   mock_get_set_vnc_config,
                   mock_enlist_image,
                   mock_set_machine_id,
                   mock_mkdir,
                   mock_create_vm,
        ...
        oslo_vim_util.get_inventory_path = mock.Mock()
        oslo_vim_util.get_inventory_path.return_value = self._dc_info.name
        ...
        with contextlib.nested(
              ...
              _get_img_svc, _get_extra_specs, _fake_metadata):
            ...
            mock_get_set_vnc_config.assert_called_once_with(
    ...
    def _test_get_spawn_vm_config_info(self,
        ...
        self.assertEqual(vi.instance.uuid, self._instance['uuid'])
    ...
    def _test_spawn_with_block_device_info(self,
                                           config_drive=False):
        block_device_info = {
    ...
    def test_spawn_with_block_device_info(self):
        self._test_spawn_with_block_device_info()

    def test_spawn_with_block_device_info_with_config_driver(self):
    ...
    @mock.patch(
        'nova.virt.vmwareapi.vmops.VMwareVMOps._create_folders',
        return_value='fake_vm_folder')
    def test_build_virtual_machine(self, mock_prepare):
        image_id = nova.tests.image.fake.get_valid_image_id()
        image = vmware_images.VMwareImage(image_id=image_id)

        extra_specs = vm_util.ExtraSpecs()
        vm_ref = self._vmops.build_virtual_machine(self._context,
                                                  self._instance,
                                                  image, self._dc_info,
        ...
        self.assertEqual('fake_uuid', vm.name)
    ...
    def test_prepare_iso_image(self, mock_generate_uuid, mock_mkdir):
    ...
    @mock.patch.object(ds_util, 'mkdir')
    @mock.patch.object(uuidutils, 'generate_uuid', return_value='tmp-uuid')
    def test_prepare_sparse_image(self, mock_generate_uuid, mock_mkdir):
    ...
    def _validate_flavor_extra_specs(self, flavor_extra_specs, expected):
        ...
        with mock.patch.object(db, 'flavor_get', _fake_flavor_get):
            ...
            flavor = flavor_obj.Flavor.get_by_id(self._context, 1)
    ...
    def _test_get_extra_specs(self, flavor_extra_specs):
        ...
        with mock.patch.object(db, 'flavor_get', _fake_flavor_get):
            # Validate that the extra specs are parsed correctly
            flavor = flavor_obj.Flavor.get_by_id(self._context, 1)
            return self._vmops._get_extra_specs(flavor)

    def test_get_storage_policy_none(self):
        self.flags(pbm_enabled=True,
                   pbm_default_policy='fake-policy', group='vmware')
        extra_specs = self._test_get_extra_specs({})
        self.assertEqual('fake-policy', extra_specs.storage_policy)

    def test_get_storage_policy_extra_specs(self):
        flavor_extra_specs = {'vmware:storage_policy': 'flavor-policy'}
        self.flags(pbm_enabled=True,
                   pbm_default_policy='default-policy', group='vmware')
        extra_specs = self._test_get_extra_specs(flavor_extra_specs)
        self.assertEqual('flavor-policy', extra_specs.storage_policy)

    def test_get_allowed_datastore_types(self):
        image_info = vmware_images.VMwareImage('fake-id',
                              disk_type=constants.DISK_TYPE_STREAM_OPTIMIZED)
        types = vmops.VMwareVMOps.get_allowed_datastore_types(image_info)
        self.assertEqual(ds_util.ALL_SUPPORTED_DS_TYPES, types)
        image_info.disk_type = constants.DISK_TYPE_SPARSE
        types = vmops.VMwareVMOps.get_allowed_datastore_types(image_info)
        self.assertEqual(ds_util.NON_STREAMABLE_DS_TYPES, types)
        image_info.file_type = constants.DISK_FORMAT_ISO
        types = vmops.VMwareVMOps.get_allowed_datastore_types(image_info)
        self.assertEqual(ds_util.ALL_SUPPORTED_DS_TYPES, types)

    def test_get_extra_config_from_flavor(self):
        flavor_extra_specs = {'vmware:extra_config':
                              '{"keyboard.typematicMinDelay":2000000}'}
        extra_specs = self._test_get_extra_specs(flavor_extra_specs)
        extra_config = {'keyboard.typematicMinDelay': 2000000}
        self.assertEqual(extra_config, extra_specs.extra_config)

    def test_get_invalid_extra_config_from_flavor(self):
        flavor_extra_specs = {'vmware:extra_config':
                              "{'keyboard.typematicMinDelay':2000000}"}
        self.assertRaises(exception.InvalidInput,
                          self._test_get_extra_specs,
                          flavor_extra_specs)
    ...
    def test_get_shadow_vm_name(self):
        image_info = vmware_images.VMwareImage(
                image_id=self._image_id,
                file_size=7,
                linked_clone=False,
        ...
        vi = vmops.VirtualMachineInstanceConfigInfo(
                self._instance, image_info,
                self._ds, self._dc_info, mock.Mock())
        name = self._vmops._get_shadow_vm_name(vi)
        expected = 'fira-10G (%s)' % self._image_id
        ...
        vi = vmops.VirtualMachineInstanceConfigInfo(
                ...
                self._ds, self._dc_info, mock.Mock())
        name = self._vmops._get_shadow_vm_name(vi)
        expected = '%s-10G (%s)' % ('X' * 37, self._image_id)
        self.assertEqual(expected, name)
