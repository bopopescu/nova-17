...
class VMwareVMUtilTestCase(test.NoDBTestCase):
    def setUp(self):
        ...
        self._instance_values = {
            ...
            'image_ref': '4', 'instance_type_id': '4',
            'availability_zone': '5', 'display_name': 'fake!',
            'uuid': instance_uuid,
...
"""
    ...
    def test_get_vmdk_path_and_adapter_type(self):
        ...
        with mock.patch.object(session, '_call_method', return_value=devices):
            vmdk = vm_util.get_vmdk_info(session, 'fake-ref')
    ...
    def test_get_vmdk_path_and_adapter_type_with_match(self):
        ...
        session = fake.FakeSession()
        with mock.patch.object(session, '_call_method', return_value=devices):
            vmdk = vm_util.get_vmdk_info(session, 'fake-ref', uuid='uuid')
            self.assertEqual('lsiLogicsas', vmdk.adapter_type)
    ...
    @mock.patch.object(utils, 'generate_password', return_value='12345678')
    def test_get_vnc_config_spec(self, mock_password):
    ...
    def _create_fake_vms(self):
        ...
        for i in range(10):
            ...
            vm.set(vm_util.VNC_CFG_PORT_KEY, opt_val)
    ...
    def test_get_vm_create_spec(self):
        ...
        expected = """{
            ...
            'numCPUs': 2}""" % {'instance_uuid': self._instance.uuid}
        expected = re.sub(r'\s+', '', expected)
        result = re.sub(r'\s+', '', repr(result))
        self.assertEqual(expected, result)
    ...
    def test_get_vm_create_spec_with_extra_config(self):
        extra_config = {'keyboard.typematicMinDelay': 2000000}
        datastore = ds_obj.Datastore('fake-ds-ref', 'fake-ds-name')
        extra_specs = vm_util.ExtraSpecs(extra_config=extra_config)
        result = vm_util.get_vm_create_spec(fake.FakeFactory(),
                                            self._instance,
                                            datastore, [],
                                            extra_specs)
    ...
    def test_get_vm_name(self):
        ...
        expected = uuid
        name = vm_util.get_vm_name(None, uuid)
        self.assertEqual(expected, name)

        display_name = 'fira-%s' % uuid
        expected = 'fira (%s)' % uuid
        name = vm_util.get_vm_name(display_name, uuid)
        self.assertEqual(expected, name)
