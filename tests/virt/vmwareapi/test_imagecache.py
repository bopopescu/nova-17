...
class ImageCacheManagerTestCase(test.NoDBTestCase):
    ...
    def test_age_cached_images(self):
        ...
        def fake_destroy_shadow_vms(image, datastore):
            self.assertEqual('fake-image-3', image)
            self.assertEqual('ds', datastore.name)
            self.assertEqual('fake-ds-ref', datastore.ref)

        with contextlib.nested(
            mock.patch.object(self._imagecache, '_destroy_shadow_vms',
                              fake_destroy_shadow_vms),
        ...
        ) as (_destroy_shadows, _get_ds_browser, _get_timestamp, _mkdir,
              _file_delete, _timestamp_cleanup):
            timeutils.set_time_override(override_time=self._time)
