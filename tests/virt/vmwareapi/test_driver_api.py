...
class VMwareAPIVMTestCase(test.NoDBTestCase):
    ...
    def _create_instance(self, node=None, set_image_ref=True,
        ...
        values = {'display_name': self._display_name,
                  'name': '000000001',
                  'id': 1,
                  'uuid': uuid,
    ...
    def _create_vm(self, node=None, num_instances=1, uuid=None,
        ...
        with mock.patch.object(oslo_vim_util, 'get_inventory_path'):
    ...
    def _get_vm_record(self):
        ...
        for vm in vms.objects:
            if vm.get('name') == vm_util.get_vm_name(self._display_name,
                                                     self.uuid):
                return vm
    ...
    def _check_vm_record(self, num_instances=1, powered_on=True):
        ...
        self.assertEqual(constants.EXTENSION_KEY,
                         vm.get("managedBy.extensionKey"))
        self.assertEqual(constants.EXTENSION_TYPE_INSTANCE,
                         vm.get("managedBy.type"))
    ...
    def test_instance_exists(self):
        self._create_vm()
        self.assertTrue(self.conn.instance_exists(self.instance['uuid']))
        self.assertFalse(self.conn.instance_exists('invalid_instance_id'))
    ...
    def test_instance_dir_disk_created(self, mock_from_image):
        ...
        img_props = vmware_images.VMwareImage(
            ...
            linked_clone=False, name='fake-name')
    ...
    def _spawn_attach_volume_vmdk(self, set_image_ref=True, vc_support=False):
        ...
        self.mox.ReplayAll()
        block_device_info = {'mount_device': root_disk}
        self.conn.spawn(self.context, self.instance, self.image,
                        injected_files=[], admin_password=None,
                        network_info=self.network_info,
    ...
    def mock_upload_image(self, context, timeout_secs, image_service, image_id,
                          owner_id, **kwargs):
        self.assertEqual(image_id, 'Test-Snapshot')
        self.assertEqual(kwargs['is_public'], False)
        self.assertEqual(kwargs['vmdk_size'], 1024)
        self.assertEqual(kwargs['image_version'], 1)
        self.assertEqual(kwargs['image_name'], "fake-snapshot-name")
        self.assertEqual(owner_id, self.project_id)
    ...
    def _test_snapshot(self, vm_ref):
        ...
        self.conn._vmops._create_linked_clone_from_snapshot(
                ...
                snapshot_ref, mox.IgnoreArg()).AndReturn(snapshot_vm_ref)

        self.mox.StubOutWithMock(vm_util, 'destroy_vm')
        vm_util.destroy_vm(self.conn._session, mox.IgnoreArg(),
                           snapshot_vm_ref)
        ...
        with mock.patch.object(nova.tests.image.fake._FakeImageService,
            ...
            with mock.patch.object(image_transfer, 'upload_image',
    ...
    def test_snapshot(self):
        self._create_vm()
        fake_vm = self._get_vm_record()
        self._test_snapshot(fake_vm.obj)
    ...
    def _test_get_vnc_console(self):
        ...
        fake_vm.set(vm_util.VNC_CFG_PORT_KEY, opt_val)
        opt_val = OptionValue(key='', value='12345678')
        fake_vm.set(vm_util.VNC_CFG_PASSWORD_KEY, opt_val)
        ...
        self.assertEqual(vnc_dict['password'], '12345678')
    ...
    def test_detach_vmdk_disk_from_vm(self):
        ...
        with contextlib.nested(
            ...
            mock.patch.object(volumeops.VMwareVolumeOps,
              ...
              get_vmdk_info, consolidate_vmdk_volume,
              detach_disk_from_vm):
...
class VMwareAPIVCDriverTestCase(VMwareAPIVMTestCase,
    ...
    def test_configuration_pbm_no_default(self, get_profile_mock):
    ...
    def test_unpause(self):
        ...
        info = self.conn.get_info({'uuid': self.uuid,
                                   'node': self.instance_node})
        self._check_vm_info(info, power_state.SUSPENDED)
        self.conn.unpause(self.instance)
        info = self.conn.get_info({'uuid': self.uuid,
                                   'node': self.instance_node})
        self._check_vm_info(info, power_state.RUNNING)
